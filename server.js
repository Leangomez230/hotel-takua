...
app.post('/api/usuarios', auth, adminOnly, function(req, res) {
  try {
    ...
    var result = db.prepare('INSERT INTO usuarios (nombre, email, password, rol) VALUES (?, ?, ?, ?)')
  ...
  try {
    ...
    if (password) {
      db.prepare('UPDATE usuarios SET nombre=?, email=?, rol=?, activo=?, password=? WHERE id=?')
        .run(nombre, email, rol, activo, bcrypt.hashSync(password, 10), req.params.id);
    } else {
...
app.put('/api/habitaciones/:id/status', auth, function(req, res) {
  try {
    ...
    db.prepare('UPDATE habitaciones SET status = ?, nota = ?, updated_at = datetime(\'now\',\'localtime\') WHERE id = ?')
      .run(status, nota !== undefined ? nota : hab.nota, req.params.id);
...
app.put('/api/habitaciones/:id', auth, adminOnly, function(req, res) {
  try {
    ...
    logAction(req.user.id, req.user.nombre, 'EDITAR_HAB', 'Hab ' + req.params.id);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/debug/habitaciones', function(req, res) {
  try {
    var habs = db.prepare('SELECT id, numero, ala, status FROM habitaciones ORDER BY ala, numero').all();
...
app.post('/api/huespedes', auth, function(req, res) {
  try {
    ...
    try {
      var r = db.prepare('INSERT INTO huespedes (documento, tipo_doc, nombre, telefono, email, nacionalidad) VALUES (?, ?, ?, ?, ?, ?)')
...
app.post('/api/checkin', auth, function(req, res) {
  try {
    ...
    var tipo_doc = req.body.tipo_doc;
    var nombre = req.body.nombre;
    var telefono = req.body.telefono;
    var entrada = req.body.entrada;
    var salida = req.body.salida;
    var noches = req.body.noches;
    var precio_total = req.body.precio_total;
    ...
    if (hab.status !== 'libre' && hab.status !== 'reservada')
      return res.status(400).json({ error: 'La habitacion esta en estado "' + hab.status + '", no se puede hacer check-in' });
 
    var huespedId = null;
    if (documento) {
      try {
        var rH = db.prepare('INSERT INTO huespedes (documento, tipo_doc, nombre, telefono) VALUES (?, ?, ?, ?)')
    ...
    var reserva = db.prepare(
      'INSERT INTO reservas (habitacion_id, huesped_id, nombre_huesped, documento, entrada, salida, noches, precio_total, metodo_pago, notas, estado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
...
app.post('/api/checkout/:habitacion_id', auth, function(req, res) {
  try {
    ...
    db.prepare('UPDATE habitaciones SET status = ?, nota = ?, updated_at = datetime(\'now\',\'localtime\') WHERE id = ?')
      .run('limpieza', '', id);
    db.prepare('UPDATE reservas SET estado = ? WHERE habitacion_id = ? AND estado = ?')
      .run('finalizada', id, 'activa');
...
app.get('/api/reservas', auth, function(req, res) {
  try {
    res.json(db.prepare(
      'SELECT r.*, h.nombre as hab_nombre, h.numero as hab_numero, h.ala, h.tipo FROM reservas r LEFT JOIN habitaciones h ON r.habitacion_id = h.id ORDER BY r.created_at DESC LIMIT 100'
    ).all());
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/reservas', auth, function(req, res) {
  try {
    var habitacion_id = req.body.habitacion_id;
    var nombre_huesped = req.body.nombre_huesped;
    var documento = req.body.documento;
    var entrada = req.body.entrada;
    var salida = req.body.salida;
    var noches = req.body.noches;
    ...
    if (!habitacion_id)  return res.status(400).json({ error: 'Falta habitacion_id' });
    if (!nombre_huesped) return res.status(400).json({ error: 'Falta el nombre del huesped' });
    if (!entrada)        return res.status(400).json({ error: 'Falta la fecha de entrada' });
    if (!salida)         return res.status(400).json({ error: 'Falta la fecha de salida' });
    ...
    if (!hab) return res.status(404).json({ error: 'Habitacion no encontrada: ' + habitacion_id });
    if (hab.status !== 'libre')
      return res.status(400).json({ error: 'La habitacion esta en estado "' + hab.status + '". Solo se puede reservar una habitacion libre.' });
 
    db.prepare(
      'INSERT INTO reservas (habitacion_id, nombre_huesped, documento, entrada, salida, noches, precio_total, metodo_pago, notas, estado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(habitacion_id, nombre_huesped, documento || '', entrada, salida, noches || 1, precio_total || 0, metodo_pago || 'Efectivo', notas || '', 'futura');
 
    db.prepare('UPDATE habitaciones SET status = ?, nota = ?, updated_at = datetime(\'now\',\'localtime\') WHERE id = ?')
      .run('reservada', nombre_huesped, habitacion_id);
 
    logAction(req.user.id, req.user.nombre, 'RESERVA', 'Hab ' + hab.numero + ' - ' + nombre_huesped);
    res.json({ ok: true });
  } catch(e) {
    console.error('RESERVA ERROR:', e);
    res.status(500).json({ error: 'Error al guardar reserva: ' + e.message });
  }
});
...
app.post('/api/caja/cerrar', auth, function(req, res) {
  try {
    ...
    db.prepare('UPDATE cajas SET estado = ?, monto_final = ?, cerrada_at = datetime(\'now\',\'localtime\') WHERE id = ?')
      .run('cerrada', final, caja.id);
...
app.post('/api/caja/movimiento', auth, function(req, res) {
  try {
    var tipo = req.body.tipo, categoria = req.body.categoria, descripcion = req.body.descripcion;
    var monto = req.body.monto, metodo_pago = req.body.metodo_pago;
    var caja = db.prepare('SELECT id FROM cajas WHERE estado = ? ORDER BY id DESC LIMIT 1').get('abierta');
    if (!caja) return res.status(400).json({ error: 'No hay caja abierta' });
...
app.get('/api/finanzas/resumen', auth, function(req, res) {
  try {
    var desde = req.query.desde, hasta = req.query.hasta;
    var movs;
    if (desde && hasta) {
      movs = db.prepare('SELECT tipo, categoria, SUM(monto) as total FROM movimientos WHERE created_at BETWEEN ? AND ? GROUP BY tipo, categoria')
        .all(desde, hasta + ' 23:59:59');
    } else {
      movs = db.prepare('SELECT tipo, categoria, SUM(monto) as total FROM movimientos GROUP BY tipo, categoria').all();
    }
    var ingresos = 0, egresos = 0;
    movs.forEach(function(m) { if (m.tipo === 'ingreso') ingresos += m.total; else egresos += m.total; });
...
app.get('/api/finanzas/movimientos', auth, function(req, res) {
  try {
    var desde = req.query.desde, hasta = req.query.hasta, tipo = req.query.tipo;
    var movs;
    if (desde && hasta && tipo) {
      movs = db.prepare('SELECT * FROM movimientos WHERE created_at BETWEEN ? AND ? AND tipo = ? ORDER BY created_at DESC LIMIT 200').all(desde, hasta + ' 23:59:59', tipo);
    } else if (desde && hasta) {
      movs = db.prepare('SELECT * FROM movimientos WHERE created_at BETWEEN ? AND ? ORDER BY created_at DESC LIMIT 200').all(desde, hasta + ' 23:59:59');
    } else {
      movs = db.prepare('SELECT * FROM movimientos ORDER BY created_at DESC LIMIT 200').all();
    }
    res.json(movs);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/productos', auth, adminOnly, function(req, res) {
  try {
    var r = db.prepare('INSERT INTO productos (nombre, categoria, precio, stock, stock_minimo) VALUES (?, ?, ?, ?, ?)')
      .run(req.body.nombre, req.body.categoria || 'general', req.body.precio, req.body.stock || 0, req.body.stock_minimo || 5);
    res.json({ id: r.lastInsertRowid });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/productos/:id', auth, adminOnly, function(req, res) {
  try {
    db.prepare('UPDATE productos SET nombre=?, categoria=?, precio=?, stock=?, stock_minimo=?, activo=? WHERE id=?')
      .run(req.body.nombre, req.body.categoria, req.body.precio, req.body.stock, req.body.stock_minimo, req.body.activo !== undefined ? req.body.activo : 1, req.params.id);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    ...
    var doVenta = db.transaction(function() {
      items.forEach(function(item) {
        ...
        var total = prod.precio * item.cantidad;
        totalVenta += total;
        db.prepare('INSERT INTO ventas_tienda (producto_id, cantidad, precio_unitario, total, caja_id, usuario_id) VALUES (?, ?, ?, ?, ?, ?)')
          .run(item.producto_id, item.cantidad, prod.precio, total, caja ? caja.id : null, req.user.id);
...
app.get('/api/dashboard', auth, function(req, res) {
  try {
    ...
    var caja = db.prepare('SELECT * FROM cajas WHERE estado = ? ORDER BY id DESC LIMIT 1').get('abierta');
    var hospedaje = 0, tienda = 0, ingresos = 0, egresos = 0;
    if (caja) {
    ...
    res.json({ habitaciones: habs, ingresos: ingresos, egresos: egresos, hospedaje: hospedaje, tienda: tienda, balance: ingresos - egresos, alertas_stock: alertas.c, caja_abierta: !!caja });
